...
from myModules_2 import stargazer,coeff,coeff_2
from myModules_3 import model_1
from imageModules import create_figure, coeff_plot,partial_plot,rsquared_plot,diagnostic
import csv
app = Flask(__name__)
@app.route("/")
def login():
...
@app.route('/sdamodel', methods = ['GET', 'POST'])
def sdamodel():
    if request.method == 'POST':
        model = request.form.get("model")
        return render_template("slr.html", model=model)
    return  render_template("sdamodel.html")
...
def sda():
    ...
    if request.method=='POST':
        ...
        if 'informational_use' and 'entertainment_use' and 'SIU' in idv:
            ...
            #Partial Plot
            partial_plot(y,X)
            return render_template('output.html',table= result_one, tables1=result_two,  tables2=[result_three.to_html(classes='output2')],
                                   tables3=[reg_metric_output.to_html(classes='output3')], dv=dv,idv=idv,coef_1=c_1,coef_2=c_2,coef_3=c_3,coef_4=c_4,std_1=s_1,std_2=s_2,
                                   std_3=s_3,std_4=s_4,pvalue_1=p_1,pvalue_2=p_2,pvalue_3=p_3,pvalue_4=p_4)
            ...
            # Partial Plot
            partial_plot(y, X)
            return render_template('output_2.html', table=result_one, tables1=result_two, tables2=[result_three.to_html(classes='output2')], tables3=[reg_metric_output.to_html(classes='output3')],
                                   dv=dv,idv=idv,coef_1=c_1,coef_2=c_2,std_1=s_1,std_2=s_2,pvalue_1=p_1,pvalue_2=p_2)
        else:
            c_1, c_2, s_1, s_2, p_1, p_2 = values(dv,idv)
            stargazer(dv, idv)
            model = request.form.get("model")
            ...
            # Partial Plot
            partial_plot(y, X)
            return render_template('output_3.html', table=result_one, tables1=result_two,tables2=[result_three.to_html(classes='output2')],tables3=[reg_metric_output.to_html(classes='output3')],
                                   dv=dv,idv=idv,coef_1=c_1,coef_2=c_2,std_1=s_1,std_2=s_2,pvalue_1=p_1,pvalue_2=p_2)
